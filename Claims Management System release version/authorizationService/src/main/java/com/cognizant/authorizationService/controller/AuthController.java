...
public class AuthController {
	...
	public ResponseEntity<?> login(@RequestBody UserData userlogincredentials) {
		logger.info("START");
		logger.info(userlogincredentials.getUserid());
		logger.info(userlogincredentials.getUpassword());
		...
		if (userdetails.getPassword().equals(userlogincredentials.getUpassword())) {
			...
			int memberId =  adminDetailService.findByUserid(uid).getMemberId();
			
			generateToken = jwtutil.generateToken(userdetails);
			logger.info(generateToken);
			logger.info("END");
			return new ResponseEntity<>(new UserData(uid, null, null, generateToken, memberId), HttpStatus.OK);
		} else {
